...
The world-customization screen's **All** page exposes every option registered by the selected profile. Boolean and enumerated values use cycling buttons; numeric, string, and list values use text fields. Numeric ranges and enumerated allowed values are validated before an edit is applied, and invalid combinations leave the selected profile unchanged.
...
| `railwayLevelOffset` | `0` (-8–2) | Raises or lowers the underground railway relative to its original level, in six-block increments. Stations, connecting slopes, adjacent railway dungeons, and building-cellar clearance use the resulting level. |
