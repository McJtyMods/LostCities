...
//        }
    ...
    private void fixTorches(BuildingInfo info) {
        ...
        for (Pair<IIndex, Map<String, Integer>> pair : torches) {
            ...
            BlockState torchState = Block.BLOCK_STATE_IDS.getByValue(torch);
            if (map != null) {
                int x = driver.getX();
                int z = driver.getZ();
// @todo 1.14
                    ...
                    for (int y = 0; y < len; y++) {
                        ...
                        if (transform != Transform.ROTATE_NONE) {
                            if (getRotatableChars().contains(b)) {
                                ...
                                bs = bs.rotate(transform.getMcRotation());
...
//                        clearRange(primer, index, lowestLevel, info.getCityGroundLevel() + info.getNumFloors() * 6, waterLevel > mainGroundLevel);
    ...
    private char getDoor(Block door, boolean upper, boolean left, net.minecraft.util.Direction facing) {
        BlockState bs = door.getDefaultState()
