...
public class ProfileConfigElement extends GuiElement {
    ...
    public void render(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(gui.getFont(), "All profile settings (" + first + "-" + last + " of " + attributes.size() + ") - scroll to browse",
                10, 43, 0xffaaaaaa);

        int labelWidth = editorX - 18;
        for (Row row : rows) {
            if (row.attribute != null) {
                graphics.text(gui.getFont(), fit(row.attribute, labelWidth), 10, row.rowY + 5,
